...
export function buildExplanation(
  ...
  if (miles > 0 && cpm > 0) {
    explanations.push(
      `${action} because net $${netStr} after ${miles.toFixed(1)} mi @ $${cpm.toFixed(2)}/mi ${comparison} required $${reqStr} to stay on pace for $${targetStr}/hr.`,
    );
  } else {
    explanations.push(
  ...
  if (result.projectedNetPerHour !== result.projectedGrossPerHour) {
    explanations.push(
      `Projected net: $${result.projectedNetPerHour.toFixed(2)}/hr (gross: $${result.projectedGrossPerHour.toFixed(2)}/hr).`,
    );
  } else {
    explanations.push(
