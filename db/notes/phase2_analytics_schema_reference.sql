...
SELECT
  ...
  COUNT(*) AS total_orders,
  SUM(CASE WHEN COALESCE(fd.final_decision, '') = 'ACCEPT' THEN 1 ELSE 0 END) AS accepted_orders,
  SUM(CASE WHEN COALESCE(fd.final_decision, '') = 'REJECT' THEN 1 ELSE 0 END) AS rejected_orders,
