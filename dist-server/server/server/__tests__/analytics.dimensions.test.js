...
import { ensureDimDriverWithClient, ensureDimZoneWithClient, ensureDimTimeWithClient, } from "../db/analytics/dimensions.js";
beforeAll(() => {
...
describe("analytics dimension helpers", () => {
    ...
    it("ensureDimTimeWithClient derives the correct time_of_day_bucket", async () => {
        ...
        // - 17:00-21:59 -> evening
        // - else        -> night
        const mkTs = (hourUtc) => new Date(Date.UTC(2024, 0, 1, hourUtc, 0, 0)); // 2024-01-01T{hour}:00:00Z
        try {
            await client.query("BEGIN");
        ...
        `, [[morningId, afternoonId, eveningId, nightId]]);
            ...
            for (const row of rows) {
                byId.set(Number(row.time_id), row.time_of_day_bucket);
            }
